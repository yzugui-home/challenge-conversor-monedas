...
// Se empleo la API Exchange Rates Data API de APILayer: https://apilayer.com/marketplace/exchangerates_data-api
...
public class TasaConversion {
	...
	public TasaConversion() {
		...
		if (this.success && this.codigoStatus == 200) {
			...
			this.conversor.put("PEN", (double) this.tasasJSON.get("PEN"));
		} else {
	...
	private void pedirTasasAPI() throws URISyntaxException, IOException, InterruptedException {
		...
		HttpRequest request = HttpRequest.newBuilder().uri(new URI(
				"https://api.apilayer.com/exchangerates_data/latest?symbols=MXN%2C%20USD%2C%20EUR%2C%20GBP%2C%20JPY%2C%20PEN&base=USD"))
				.header("apikey", "WY5MFEvP6XFQkPXbhk7g1zmvVyk9vBOs").timeout(Duration.ofSeconds(30)).GET().build();
